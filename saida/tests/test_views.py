...
from estoque.models import Produto, Fornecedor, Saida
...
class TestViews(TestCase):
  ...
  def test_saida_POST(self):
    ...
    response = self.client.post(
      self.saida_url,
      {
        'produto': self.produtos[0].id,
        'quantidade': 1
      })

    self.saidas = Saida.objects.all()

    self.assertEquals(response.status_code, 302)
    self.assertEquals(self.saidas.count(), 1)
    self.assertEquals(self.produtos[0].estoque, 9)